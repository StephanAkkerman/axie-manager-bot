    ...
    async def new_listings(self):
        ...
        try:
            ...
            try:
                ...
                df["morale"] = pd.to_numeric(df["stats"].apply(lambda x: x["morale"]))
                df["speed"] = pd.to_numeric(df["stats"].apply(lambda x: x["speed"]))
                df["skill"] = pd.to_numeric(df["stats"].apply(lambda x: x["skill"]))
            except Exception as e:
                print(e)
