...
class Activity(commands.Cog):
    ...
    async def get_axie_auctions(self):
        ...
        for address in addresses:
            ...
            owned_axies['Manager'] = address_df.loc[address_df['Address'] == address]['Manager'].tolist()[0]
        ...
        # Compare
            ...
            # No difference in ids
                ...
                old_auctions = self.axie_db.loc[self.axie_db['auction'] != None]['id'].tolist()
    ...
    async def send_msg(self, df, id, keyword):
        ...
        if 'price' in row.columns:
            e.add_field(
                        name="Price",
                        value=f"${str(row['price'].tolist()[0])}\n",
                        inline=True,
                        )
    ...
    async def get_addresses(self):
        ...
        scholars = (
            gd.get_as_dataframe(sheet.worksheet("Scholars")).dropna(axis=0, how="all").dropna(axis=1, how="all")
        )
        funds = (
            gd.get_as_dataframe(sheet.worksheet("Funds")).dropna(axis=0, how="all").dropna(axis=1, how="all")
        )
