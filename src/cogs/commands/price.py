...
class Price(commands.Cog):
    ...
    async def price(self, ctx, *input):
        ...
        price1, id1, price2, id2, price3, id3, price4, id4 = await self.get_axie_info(
            axie_id
        )
        ...
        e = discord.Embed(
            title=f"Recommended price for selling axie #{axie_id}",
            description="",
            color=0x00FFFF,
            url=f"https://www.axieinfinity.com/axie/{axie_id}/",
        )
        e.set_foot(
            text=f"[Link to axie.tech pricing](https://www.axie.tech/axie-pricing/{axie_id})"
        )

        if id1 != 0:
            e.add_field(
                name="Class, Breedcount, Abilities",
                value=f"[${price1} / Ξ{round(float(price1)/float(eth_price),3)}](https://www.axieinfinity.com/axie/{id1}/)",
                inline=False,
            )
        if id2 != 0:
            e.add_field(
                name="+ Stats",
                value=f"[${price2} / Ξ{round(float(price2)/float(eth_price),3)}](https://www.axieinfinity.com/axie/{id2}/)",
                inline=False,
            )
        if id3 != 0:
            e.add_field(
                name="+ R1 (abilities)",
                value=f"[${price3} / Ξ{round(float(price3)/float(eth_price),3)}](https://www.axieinfinity.com/axie/{id3}/)",
                inline=False,
            )
        if id4 != 0:
            e.add_field(
                name="+ R1 (all)",
                value=f"[${price4} / Ξ{round(float(price4)/float(eth_price),3)}](https://www.axieinfinity.com/axie/{id4}/)",
                inline=False,
            )
    ...
    async def get_axie_info(self, axie_id):
        ...
        try:
            (
                ...
                skill,
                morale,
            ) = await self.formatted_axie_info(axie_id)
        except Exception:
            return 0, 0, 0, 0, 0, 0, 0, 0

        # Start with simple prices
        # 1: Price is based on abilities, class and breedcount
        price1, id1 = await self.simple_prices(
            1, classes, breedCount, parts1, hp, speed, skill, morale
        )
        # 2: Price is also based on the stats
        price2, id2 = await self.simple_prices(
            2, classes, breedCount, parts1, hp, speed, skill, morale
        )

        # Get more advanced price indicators
        # 3: Price is also based on r1 genes of abilities
        price3, id3 = await self.gene_prices(
            3, axie_id, classes, breedCount, parts1, hp, speed, skill, morale
        )
        # 4: Price is also based on all R1 genes
        price4, id4 = await self.gene_prices(
            4, axie_id, classes, breedCount, parts2, hp, speed, skill, morale
        )

        return price1, id1, price2, id2, price3, id3, price4, id4

    async def simple_prices(
        self, price: int, classes, breedCount, parts, hp, speed, skill, morale
    ):
        try:
            if price == 1:
                response = await self.get_old_listings(classes, breedCount, parts)
                return (
                    response.head(1)["auction"].tolist()[0]["currentPriceUSD"],
                    response.head(1)["id"].tolist()[0],
                )
            if price == 2:
                response = await self.get_old_listings(
                    classes, breedCount, parts, hp, speed, skill, morale
                )
                return (
                    response.head(1)["auction"].tolist()[0]["currentPriceUSD"],
                    response.head(1)["id"].tolist()[0],
    ...
    async def gene_prices(
        self, price: int, axie_id, classes, breedCount, parts, hp, speed, skill, morale
    ):
        ...
        try:
            ...
            listings = await self.get_old_listings(
                classes, breedCount, parts, hp, speed, skill, morale, start
            )
    ...
    def r1_all(self, axie_genes, genes):
        ...
        return genes.loc[
            (genes["eyes r1"] == (axie_genes["eyes r1"].tolist()[0]))
    ...
    async def formatted_axie_info(self, axie_id):
        ...
        df["morale"] = pd.to_numeric(df["stats"].apply(lambda x: x["morale"])).tolist()[
            0
        ]
    ...
    async def get_axie_genes(self, axie_id):
        ...
        for part in ["eyes", "ears", "mouth", "horn", "back", "tail"]:
            try:
                genes[f"{part} r1"] = genes["traits"].apply(
                    lambda x: x[part]["r1"]["partId"]
                )
                genes[f"{part} r2"] = genes["traits"].apply(
                    lambda x: x[part]["r2"]["partId"]
                )
            except Exception as e:
                print(e)
