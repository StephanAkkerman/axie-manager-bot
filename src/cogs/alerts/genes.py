...
def get_genes(axie_df, r1, r2):
    ...
    try:
        response = requests.get(
            "https://api.axie.technology/getgenes/" + ids
        ).json()
    ...
    genes['r1 deviation'] = genes['mouth r1'] + genes['horn r1'] + genes['back r1'] + genes['tail r1']
    genes['r2 deviation'] = genes['mouth r2'] + genes['horn r2'] + genes['back r2'] + genes['tail r2']

    # Only get the axies where deviations are lower than what we asked for
    genes = genes.loc[(genes['r1 deviation'] <= r1) &
                      (genes['r2 deviation'] <= r2)
                     ]
    ...
    print(f"Found these axies with good genes: {ids}")
